...
$(document).ready(function() {
    ...
    $("#slider-range").slider({
        ...
        slide: function(event, ui) {
            $("#amount").val(ui.values[0] + "VNĐ" + " - " + ui.values[1] + " VNĐ"); // Đưa " VNĐ" ra sau giá trị

            // Load data when the slider is moved
            loadData(ui.values[0], ui.values[1]);
        }
    });
    // Set initial values for the input field
    $("#amount").val($("#slider-range").slider("values", 0) +
        " - " + $("#slider-range").slider("values", 1) + " VNĐ"); // Đưa " VNĐ" ra sau giá trị
});
function loadData(minPrice, maxPrice) {
        // Use AJAX to load data based on the slider values
        $.ajax({
            url: '../controller/filterByPrice.php', // Replace with your actual data endpoint
            method: 'POST',
            data: {
                minPrice: minPrice,
                maxPrice: maxPrice
            ...
            success: function (data) {
                // Update the product list with the loaded data
        ...
        success: function(data) {
            ...
            if (data.review_data.length > 0) {
                ...
                for (var count = 0; count < data.review_data.length; count++) {
                    ...
                    html += '<div class="username">' + data.review_data[count].username.charAt(0)
                        .toUpperCase() + '</div>';
                    html += '<div class="review-info">';
                    html += '<div class="stars">';
                    // Display star ratings
                    for (var star = 1; star <= 5; star++) {
                        var class_name = (data.review_data[count].rating >= star) ?
                            'fa fa-star star-active' : 'fa fa-star-o';
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(product => {
                        ...
                        html +=
                            '<a class="searchPro" href="./indexProdetail.php?id=' +
                            product.product_id + '&act=loadOne&name=' + product
                            .category_name + '">' + product.product_name + '</a>';
                        html += '<div class="product-contents">';
                        html +=
                            '<div class="product-image"><img width="50" height="50" src="../images/' +
                            product.product_image + '" alt="' + product
                            .product_name + '"></div>';
                        html += '<div class="search-info">';
                        html +=
                            '<span class="product-name"><a href="./indexProdetail.php?id=' +
                            product.product_id + '&act=loadOne&name=' + product
                            .category_name + '">' + product.product_name +
                            '</a></span>';
                        html += '<span class="product-price">' + product
                            .product_price + "VNĐ" + '</span>';
                        html += '</div>';
                        html += '</div>';
                        html += '</a>';
                        html += '</li>';
...
$(".buyNow").click(function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // alert("Product added successfully");
            // location.reload();
            window.location.href = '../views/indexCheckout.php';
            console.log(response);
            // Thực hiện các thao tác khác tùy thuộc vào dữ liệu nhận được
        },
        error: function(error) {
...
$(".update-button").click(function(e) {
    ...
    $('tbody tr').each(function() {
        ...
        console.log(productId, productName, productImage, productPrice, productQuantity, productPrice *
            productQuantity, newTotalProduct, inputValue);
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                method: 'post',
                data: {
                    items: JSON.stringify(itemsToRemove)
                },
                success: function(response) {
                    Swal.fire(
                        'Deleted!',
